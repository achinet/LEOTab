...
extension LEOTableViewController : LEOScrollViewProtocol {
    ...
    public func willDisplayCell(notif: NSNotification) {
        ...
        if let letLeoNavigationBar = self.leoNavigationBar {
            ...
            if (self.tableView.contentSize.height < UIScreen.mainScreen().bounds.size.height - (leoNavigationBar?.getBackLayerMaxHeight())! - self.tableView.contentInset.bottom) { return }
